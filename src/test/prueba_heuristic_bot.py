...
class HeuristicPlayer(Player):
    ...
    def choose_move(self, battle):
        ...
        if battle.available_moves:
            ...
            if contextual_score <= -1 and battle.available_switches != [] and not isinstance(self.historic_moves[-1],Pokemon): # If the score is negative, we change the active pokemon
                best_move = self.best_switch_action(battle.available_switches, opp_mon, debug)
                self.historic_moves.append(best_move)
    ...
    def choose_best_move(self, battle, opp_mon, terastallized : bool):
        ...
        if terastallized:
            if all(move.type.damage_multiplier(opp_mon.tera_type,type_chart=GenData.from_gen(8).type_chart) < 1 for move in battle.available_moves) and battle.available_switches != [] and not isinstance(self.historic_moves[-1],Pokemon):
            ...
            if all(move.type.damage_multiplier(*opp_mon.types,type_chart=GenData.from_gen(8).type_chart) < 1 for move in battle.available_moves) and battle.available_switches != [] and not isinstance(self.historic_moves[-1],Pokemon):
                return self.best_switch_action(battle.available_switches, opp_mon, False)
